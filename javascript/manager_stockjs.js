...
// --- Dummy Stock Data ---
const stockData = [
    { name: "Kesar Pista Kulfi", category: "Kulfi", price: 40, qty: 500, status: "in-stock" },
    { name: "Malai Kulfi", category: "Kulfi", price: 30, qty: 120, status: "in-stock" },
    { name: "Mango Dolly", category: "Ice Cream", price: 20, qty: 25, status: "low" },
    { name: "Chocolate Cone", category: "Cone", price: 50, qty: 0, status: "out" },
    { name: "Rabdi Falooda", category: "Kulfi", price: 60, qty: 200, status: "in-stock" }
];
...
function renderTable(data) {
    ...
    data.forEach(item => {
        ...
        if (item.status === 'low') { statusText = "Low Stock"; statusClass = "low"; }
        if (item.status === 'out') { statusText = "Out of Stock"; statusClass = "out"; }
...
if(addBtn) addBtn.addEventListener('click', () => openModal(stockModal));
if(assignBtn) assignBtn.addEventListener('click', () => openModal(assignModal));
...
closeButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        if(modalToClose) closeModalFunction(modalToClose);
    });
});

// Close on outside click (Overlay logic handled by individual modals)
...
// --- FORM SUBMIT LOGIC (Dummy Alerts) ---
if(addForm) {
